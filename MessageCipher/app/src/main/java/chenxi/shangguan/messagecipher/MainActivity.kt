...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ui.btnPublicKeyEncrypt.setOnClickListener {
            ...
            if (aes_pwd.length != 16 && aes_pwd.length != 24 && aes_pwd.length != 32){
                AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("密码应为16/24/32字符长度").create().show()
            }else{
                if (public_key != ""){
                    ui.txtEncryptedTxt.setText(encryptByPublicKey(aes_pwd, public_key))
                }else{
                    AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("需要提供公钥").create().show()
                }
            }
        }
        ...
        ui.btnPublicKeyDecrypt.setOnClickListener {
            ...
            if (encrpyed_text == "") {
                AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("需要提供密文").create().show()
            }else{
                if (public_key != ""){
        ...
        ui.btnPrivateKeyEncrypt.setOnClickListener {
            ...
            if (aes_pwd.length != 16 && aes_pwd.length != 24 && aes_pwd.length != 32){
                ...
                if (private_key != ""){
                    ...
                    AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("需要提供私钥").create().show()
                }
            }
        }
                ...
                if (encrpyed_text == "") {
                    if (aes_pwd.length != 16 && aes_pwd.length != 24 && aes_pwd.length != 32) {
                        AlertDialog.Builder(this@MainActivity).setTitle("错误")
                                .setMessage("密码应为16/24/32字符长度").create().show()
                    } else {
                        ui.txtEncryptedTxt.setText(encryptByPrivateKey(aes_pwd, private_key))
                        ...
                        try {
                            ...
                            AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("已完成加密和签名并已复制到剪贴板").create().show()
                        } catch (e: Exception) {
                    ...
                    AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("已完成签名并已复制到剪贴板").create().show()
                }
            }else{
                AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("需要提供私钥").create().show()
            }
        ...
        ui.btnVerifySignature.setOnClickListener {
            ...
            if (encrpyed_text == "") {
                ...
                AlertDialog.Builder(this@MainActivity).setTitle("错误").setMessage("需要提供数字签名").create().show()
            }else{
                if (verify(encrpyed_text, public_key, signature_text)){
                    ui.txtAESPwd.setText(decryptByPublicKey(encrpyed_text, public_key))
                    AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("合法签名，验证成功").create().show()
                }else{
                    AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("签名不合法，验证失败").create().show()
                }
            }
        }
        ...
        ui.btnCopyPublicKey.setOnClickListener {
            ...
            try {
                ...
                // 将ClipData内容放到系统剪贴板里。
                cm.setPrimaryClip(mClipData);
                AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("公钥已复制到剪贴板").create().show()
            } catch (e: Exception) {
        ...
        ui.btnCopyPrivateKey.setOnClickListener {
            ...
            try {
                ...
                // 将ClipData内容放到系统剪贴板里。
                cm.setPrimaryClip(mClipData);
                AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("私钥已复制到剪贴板").create().show()
            } catch (e: Exception) {
        ...
        ui.btnCopyEncryptedTxt.setOnClickListener {
            ...
            try {
                ...
                // 将ClipData内容放到系统剪贴板里。
                cm.setPrimaryClip(mClipData);
                AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("密文已复制到剪贴板").create().show()
            } catch (e: Exception) {
        ...
        ui.btnPurgeAll.setOnClickListener {
            ...
            AlertDialog.Builder(this@MainActivity).setTitle("提示").setMessage("重置成功").create().show()
        }
        ...
        ui.btnGenRandAesPwd.setOnClickListener {
            ...
            ui.txtEncryptedTxt.text.clear()
            ui.txtSignature.text.clear()
            ui.txtAESPwd.setText(createRandom(false, 16))
        }
    ...
    fun createRandom(numberFlag: Boolean, length: Int): String? {
        var retStr = ""
        val strTable = if (numberFlag) "1234567890" else "`1234567890-=abcdefghijklmnopqrstuvwxyz~!@#$%^&*()_+[]{}ABCDEFGHTJKLMNOPQRSTUVWXYZ;:,./<>?"
    ...
    fun decryptByPrivateKey(data: String?, key: String?): String? {
        try {
            ...
            val original = cipher.doFinal(Base64.decode(data?.toByteArray(), Base64.DEFAULT))
